...
uniform samplerCube shadowCubeMaps[POINT_LIGHT_COUNT];
...
float calcShadowDirLight(sampler2DShadow shadowMap, vec4 fragPosLightSpace, vec3 lightDir, vec3 norm)
{
...
vec3 calcDirLight(DirLight light, vec3 normal, vec3 viewDir)
{
    ...
    float shadow = calcShadowDirLight(dirLightShadowMap, fs_in.fragPosLightSpace, lightDir, normal);

    return ambient + ((1.0 - shadow) * (diffuse + specular));
}

float calcShadowPointLight(samplerCube cubeMap, vec3 lightDir, vec3 normal)
{
...
vec3 calcPointLight(int lightIndex, vec3 normal, vec3 fragPosition, vec3 viewDir)
{
    ...
    lightDir = fragPosition - light.position;
    float shadow = calcShadowPointLight(shadowCubeMaps[lightIndex], lightDir, normal);
