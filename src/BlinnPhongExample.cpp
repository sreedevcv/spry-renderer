...
private:
    ...
    void onUpdate(float deltaTime) override
    {
        ...
        dirLight.direction.x = lightPosition.x + 10.0f * glm::sin(spry::getGlobalTime());
        // dirLight.direction.y = lightPosition.y + 10.0f * glm::cos(spry::getGlobalTime());
        dirLight.direction.z = lightPosition.z + 10.0f * glm::sin(spry::getGlobalTime());
        ...
        for (int i = 0; i < POINT_LIGHT_COUNT; i++) {
            glm::vec3 newPos;
            newPos.x = pointLights[i].position.x + 10.0f * glm::sin(spry::getGlobalTime());
            newPos.y = pointLights[i].position.y + 10.0f * glm::cos(spry::getGlobalTime());
            newPos.z = pointLights[i].position.z + 10.0f * glm::sin(spry::getGlobalTime());
